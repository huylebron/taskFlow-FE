...
function BoardContent({ board, moveColumns, moveCardInTheSameColumn, moveCardToDifferentColumn }) {
  ...
  const renderBoardBackground = () => {
    // Default styling for the board container
    const backgroundStyles = {
      backgroundImage: 'linear-gradient(to right, #0079bf, #1565c0)',
      backgroundSize: 'cover',
      backgroundPosition: 'center',
      transition: 'background 0.3s ease'
    };

    // Apply background based on board settings
    if (board?.backgroundType === 'image' && board?.backgroundImage) {
      backgroundStyles.backgroundImage = `url(${board.backgroundImage})`;
    } else if (board?.backgroundType === 'color' && board?.backgroundColor) {
      if (board.backgroundColor.includes('gradient')) {
        backgroundStyles.backgroundImage = board.backgroundColor;
      } else {
        backgroundStyles.backgroundImage = 'none';
        backgroundStyles.backgroundColor = board.backgroundColor;
      }
    }
    ...
    >
      ...
      >
        <ListColumns
          ...
          createNewColumn={board.createNewColumn}
          createNewCard={board.createNewCard}
        />
